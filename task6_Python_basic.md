...
1.1 下载地址：https://pypi.org/project/pygame/#files
根据安装Python版本、系统X64 & X32 确定需要的packages

1.2 模块安装：启动Windows命令行，切换目录至模块所在路径，运行命令：
`python -m pip install --user pygame-1.9.6-cp37-cp37m-win_amd64.whl`

等待提示“Successfully installed pygame” ,即完成安装
网上教程提示“pip version ”更新问题，经后续测试，不更新并不影响程序的运行

1.3检测模块是否可用：
命令行运行`python` 进入Python 命令行界面， 输入`import pygame`
出现提示：
`>>> import pygame
pygame 1.9.6
